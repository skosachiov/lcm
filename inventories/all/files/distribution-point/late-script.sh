...
if ! grep -q "lcm" "/etc/cron.d/ansible-pull"; then
	echo '@reboot root bash -c "sleep 60 && uname -a | logger -d -n '${fqdn}'"' > /etc/cron.d/ansible-pull
	echo '@reboot root bash -c "sleep 60 && /usr/bin/ansible-pull -i localhost -U https://'${fqdn}'/git/lcm | logger -d -n '${fqdn}'"' >> /etc/cron.d/ansible-pull
